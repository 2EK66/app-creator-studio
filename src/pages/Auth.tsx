...
export default function Auth() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAlert({ type: "error", msg: err.message || "Une erreur est survenue" });
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-mirec-900 via-mirec-800 to-mirec-700 flex flex-col items-center justify-center px-4 py-8">
      <div className="bg-card rounded-3xl p-8 w-full max-w-[400px] shadow-2xl">
        ...
        <div className="space-y-3">
          {mode === "register" && (
            <InputField label="Nom complet" value={fullName} onChange={setFullName} placeholder="Jean-Baptiste Koffi" error={errors.fullName} />
          )}
          <InputField label="Email" type="email" value={email} onChange={setEmail} placeholder="ton@email.com" error={errors.email} />
          {mode !== "forgot" && (
            <div className="relative">
              <InputField label="Mot de passe" type={showPw ? "text" : "password"} value={password} onChange={setPassword} placeholder="••••••••" error={errors.password} />
              <button type="button" onClick={() => setShowPw((s) => !s)} className="absolute right-3 top-8 text-muted-foreground hover:text-foreground">
                {showPw ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
            </div>
          )}
          {mode === "register" && (
            <InputField label="Confirmer" type="password" value={confirm} onChange={setConfirm} placeholder="••••••••" error={errors.confirm} />
          )}
          {mode === "login" && (
            <button onClick={() => switchMode("forgot")} className="text-xs text-primary hover:underline ml-auto block">
              Mot de passe oublié ?
            </button>
          )}
          <button
          ...
          >
            {loading ? "Chargement…" : mode === "login" ? "Se connecter" : mode === "register" ? "Rejoindre MIREC" : "Envoyer le lien"}
          </button>
          {mode === "forgot" && (
            <button onClick={() => switchMode("login")} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground mx-auto mt-2">
        ...
        {mode !== "forgot" && (
          <p className="text-center text-[11px] text-muted-foreground mt-6 font-display italic leading-relaxed">
            "Car là où deux ou trois sont réunis en mon nom,<br />je suis au milieu d'eux." — Mat. 18:20
          </p>
        )}
      </div>
