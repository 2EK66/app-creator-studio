    ...
    return (
      <div className="min-h-screen bg-background flex flex-col pb-20">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 max-w-lg mx-auto w-full space-y-3">
          {messages.length === 0 && (
            <p className="text-center text-muted-foreground text-sm py-12">Aucun message encore. Sois le premier !</p>
          )}
          {messages.map((msg) => {
      ...
      <header className="sticky top-0 z-30 bg-card/80 backdrop-blur-lg border-b border-border/50 px-4 py-3">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="font-display text-xl font-bold text-foreground">Groupes</h1>
          </div>
          <button onClick={() => setShowCreate(true)} className="p-2 rounded-full hover:bg-muted transition-colors">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-1.5 ${
                  newGroupVisibility === "public" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium flex items-center justify-center gap-1.5 ${
                  newGroupVisibility === "private" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                }`}
              >
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground text-xs mt-1">Crée le premier !</p>
          </div>
        ) : (
          groups.map((group) => (
            <div
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1 min-w-0" onClick={() => openGroup(group)}>
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      ...
                      {group.visibility === "private" && " · Privé"}
                    </p>
                  </div>
                </div>
                {group.is_member ? (
