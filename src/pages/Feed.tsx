...
function PostAvatar({ avatarUrl, initials, size = 40 }: { avatarUrl: string | null; initials: string; size?: number }) {
  const [imgError, setImgError] = useState(false);
  if (avatarUrl && !imgError) {
    return (
      <img src={avatarUrl} alt={initials} onError={() => setImgError(true)}
        style={{ width: size, height: size }}
        className="rounded-full object-cover flex-shrink-0 border-2 border-border/30" />
    );
  }
  return (
    <div style={{ width: size, height: size, backgroundColor: "hsl(220 70% 35%)" }}
...
export default function Feed() {
  ...
  const handleNewPost = async (data: { content: string; type: string }) => {
    ...
    if (error) { console.error("Erreur post:", error); return; }
    fetchPosts();
  };
          ...
          return (
            <div key={post.id} className="bg-card border border-border/50 rounded-2xl p-4 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                <PostAvatar avatarUrl={post.author_avatar} initials={post.author_initials} size={40} />
              ...
              <div className="flex gap-2 pt-3 border-t border-border/30">
                {([{ key: "amen" as const, emoji: "🙏", label: "Amen" }, { key: "feu" as const, emoji: "🔥", label: "Feu" }, { key: "coeur" as const, emoji: "❤️", label: "Cœur" }]).map(r => (
