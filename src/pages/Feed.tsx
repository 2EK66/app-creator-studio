...
// ============================================================
// PROPS — reçoit onTabChange pour naviguer sans react-router
// ============================================================
interface FeedProps {
...
function PostAvatar({ avatarUrl, initials, size = 40, onClick }: {
  ...
  if (avatarUrl && !imgError) {
    return (
      <button onClick={onClick}
        className="flex-shrink-0 rounded-full overflow-hidden border-2 border-border/30 hover:border-primary/60 hover:scale-105 transition-all focus:outline-none"
        style={{ width: size, height: size }} title="Voir la photo">
  ...
  return (
    <button onClick={onClick}
      ...
      className="rounded-full flex items-center justify-center flex-shrink-0 hover:brightness-110 hover:scale-105 transition-all focus:outline-none">
...
// — bouton "Envoyer un message" utilise onTabChange (pas navigate)
// ============================================================
function UserProfileModal({ userId, name, initials, avatarUrl, onClose, onViewPhoto, onTabChange }: {
  ...
  useEffect(() => {
    ...
    supabase.from("posts")
      .select("id", { count: "exact", head: true }).eq("author_id", userId)
      .then(({ count }) => { if (count !== null) setPostCount(count); });

    (supabase.from("member_skills" as any)
      .select("skill, level").eq("profile_id", userId).limit(5) as any)
  ...
  const handleSendMessage = () => {
    onClose();
    // Naviguer vers l'onglet inbox ET ouvrir la conversation
    if (onTabChange) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 flex items-end justify-center" onClick={onClose}>
      <div className="bg-card w-full max-w-lg rounded-t-3xl overflow-hidden"
        ...
        {/* Bouton message — utilise onTabChange, PAS navigate */}
  ...
  const fetchPosts = useCallback(async (isRefresh = false) => {
    ...
    if (authorIds.length > 0) {
      ...
      profileMap = Object.fromEntries(
        (profiles || []).map(p => [p.id, { name: p.full_name || "Membre MIREC", avatar: getAvatarUrl(p.avatar_url) }])
      );
    }

    const postIds = postsData.map(p => p.id);
    const { data: allReactions } = await supabase.from("reactions")
      .select("content_id, type, author_id").in("content_id", postIds);
    const { data: allComments } = await supabase.from("comments")
    ...
    const enriched: FeedPost[] = postsData.map(post => {
      ...
      return { ...post, type: post.type || "post", author_name: name, author_initials: name.slice(0, 2).toUpperCase(), author_avatar: p?.avatar || null, reactions, user_reactions, comments_count: commentCounts[post.id] || 0 };
    });
          ...
          return (
            <div key={post.id} className="bg-card border border-border/50 rounded-2xl p-4 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${tc.textClass} ${tc.bgClass}`}>{tc.label}</span>

                {isAuthor && (
                  <div className="relative">
                    <button onClick={() => setMenuOpen(menuOpen === post.id ? null : post.id)} className="p-1.5 rounded-full hover:bg-muted transition-colors">
              ...
              <div className="flex gap-2 pt-3 border-t border-border/30">
                {([{ key: "amen" as const, emoji: "🙏" }, { key: "feu" as const, emoji: "🔥" }, { key: "coeur" as const, emoji: "❤️" }]).map(r => (
                  <button key={r.key} onClick={() => handleReact(post.id, r.key)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                      post.user_reactions[r.key] ? "bg-primary/10 text-primary border border-primary/20" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
                    <span>{r.emoji}</span><span>{post.reactions[r.key]}</span>
                  </button>
                ))}
              </div>
