...
export default function Podcast() {
  ...
  const [isCreator, setIsCreator] = useState(false); // AJOUTÉ
  ...
  // Vérifier si l'utilisateur a déjà un canal (est créateur)
  useEffect(() => {
    if (!user) return;
  ...
  // Récupérer le statut de la demande créateur
  useEffect(() => {
    if (!user) return;
      ...
      <div className="sticky top-0 z-30 px-4 py-3" style={{ background:"rgba(13,8,32,0.9)", backdropFilter:"blur(16px)", borderBottom:"1px solid rgba(139,92,246,0.15)" }}>
        <div className="max-w-lg mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* BOUTON ESPACE CRÉATEUR (si l'utilisateur a un canal) */}
            {isCreator && (
            ...
            {/* Bouton Créateur / statut existant */}
